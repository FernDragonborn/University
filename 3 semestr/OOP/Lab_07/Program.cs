...
using static Lab_07.misc.Messages;
...
internal class Program
{
    public static void Main()
    {
        ...
        while (true)
        {
            ...
            if (state == State.GetName)
            {
                ...
                if (input == "1")
                {
                    Console.WriteLine("Введіть рядок для парсингу");
                    string[] sArr = new string[9];
                    sArr[0] = Console.ReadLine();
                    sArr[1] = Console.ReadLine();
                    sArr[2] = Console.ReadLine();
                    sArr[3] = Console.ReadLine();
                    sArr[4] = Console.ReadLine();
                    sArr[5] = Console.ReadLine();
                    sArr[6] = Console.ReadLine();
                    sArr[7] = Console.ReadLine();
                    sArr[8] = Console.ReadLine();
                    if (sArr.Any(x => x == null || x.Length == 0))
                    {
                        Console.WriteLine("Не вдалося спарсити, вас перенесено до меню створення персонажа\n(введіть 1, щоб знову спробувати спарсити рядок)");
                        continue;
                    }

                    string s = string.Join(" \n", sArr);
                    var p = Player.Parse(s);

                    if (!handler.TryParsePlayer(s))
                    {
                        Console.WriteLine("Гравець із таким айді вже існує. Введеному гравцю було присвоєне інше");
                    }

                    WriteMessageMenuAndSetDefaultState(out state, $"додано гравця з іменем {p.InGameName}");
                    continue;
                }
            ...
            else if (state == State.ChoosePlayer)
            {
                if (input == "0") { WriteMessageMenuAndSetDefaultState(out state, "Ви повернулись до головного меню"); continue; }
            ...
            else if (state == State.CustomInteraction)
            {
                if (input == "0") { WriteMessageMenuAndSetDefaultState(out state, $"Ви повернулись до головного меню"); continue; }
            ...
            else if (state == State.DeletePlayer)
            {
                if (input == "0") { WriteMessageMenuAndSetDefaultState(out state, $"Ви повернулись до головного меню"); continue; }
                Player? player;
                try
            ...
            else if (state == State.SaveCollection)
            {
                string path = null;
                string message = "";
                if (input == "0") { WriteMessageMenuAndSetDefaultState(out state, $"Ви повернулись до головного меню"); continue; }
                ...
                {
                    path = "players_collection.csv";
                    handler.SaveToFileCSV(path, out message);
                }
                else if (input == "2")
                {
                    path = "players_collection.json";
                    handler.SaveToFileJson(path, out message);
                }
                else
                {
                    WriteError();
            ...
            else if (state == State.ReadCollection)
            {
                string message = "";
                if (input == "0") { WriteMessageMenuAndSetDefaultState(out state, $"Ви повернулись до головного меню"); continue; }
                if (input.EndsWith(".csv")) handler.ReadFromFileCsv(input, out message);
                else if (input.EndsWith(".json")) handler.ReadFromFileJson(input, out message);
                WriteMessageMenuAndSetDefaultState(out state, message);
                continue;
            }
            ...
            switch (numInput)
            {
                ...
                case 1:
                    state = State.GetName;
                    Console.WriteLine("Введіть \"1\" для того, щоб розпарсити гравця із рядка,\nабо напишіть ім'я гравця, щоб створити нового: ");
                    continue;
                ...
                //Save to file
                case 5:
                    state = State.SaveCollection;
                    Console.WriteLine("Оберіть тип файлу для збереження:\n1. csv\n2. json");
                    continue;
                //Add objects to collection reading from file
                case 6:
                    state = State.ReadCollection;
                ...
                case 7:
                    ...
                    WriteMessageMenuAndSetDefaultState(out state, $"Коллекція очищена. Тепер у списку {handler.count} гравців");
                    continue;
                default:
                    WriteErrorMenuAndSetDefaultState(out state);
                    continue;
            }
        }
    }

    private static string WriteStateAndApplyInteraction(Player chosenPlayer, Lab_07.misc.Interaction interaction)
    {
    ...
    {
        Console.Clear();
        Console.WriteLine("Меню\n1: Додати нового гравця\n2: Вивести існуючих гравців\n3: Застосувати дію конкретного гравця за айді або ім'ям\n4: Видалити гравця\n5: Зберегти колекцію у файл\n6: Прочитати дані з файлу\n7: Очистити колекцію\n0: Вийти або назад\n\n");
    }
}
